...
public class Game {
    ...
    public static void main(String[] args) {
        ...
        if (state.getState() == "won") {
            System.out.println("You won the great KTH Indamon game!");
        } else if (state.getState() == "lost") {
    ...
    private static void generateRooms() {
        ...
        Room start = new Room(new Indamon("Isak", 1, 1, 1), "You are at the Tekniska Högskolan subway station. You look around for " +
                "someone handing out free samples, but there don't seem to be any today.");
        Room borggarden = new Room(new Indamon("Charizard", 5, 5, 5), "You are outside in borggården, there is a cold wind blowing.");
        Room f1 = new Room(new Indamon("Pikachu", 5, 5, 5), "You are in the main F1 lecture hall, there is a very advanced " +
                "math lecture going on. You don't understand anything.");
        Room e43 = new Room(new Indamon("Johlini", 5, 5, 5), "You are in an empty classroom in the E building. Someone has written " +
                "\"I love INDA\" on the blackboard");
        Room gul = new Room(new Indamon("Glassey", 5, 6, 7), "You are in the Gul computer lab. Someone has left a computer running with " +
        ...
        state = new GameState(start, new Indamon("Johlini Hahlini Tortellini", 10, 1, 1));
    }
